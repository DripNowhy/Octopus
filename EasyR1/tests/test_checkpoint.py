...
def test_remove_obsolete_ckpt_with_protected(save_checkpoint_path):
    ...
    remove_obsolete_ckpt(
        ...
        best_global_step=10,
        save_limit=4,
        protected_steps=[15, 20],
    )

    expected = {10, 15, 20}
    for step in range(5, 35, 5):
        should_exist = step in expected
        assert os.path.exists(os.path.join(save_checkpoint_path, f"global_step_{step}")) == should_exist
