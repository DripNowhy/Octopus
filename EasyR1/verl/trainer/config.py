...
class TrainerConfig:
    ...
    save_metric: str = "reward"
    """metric used to decide which checkpoints to keep (`reward`, `accuracy_sampled`, `accuracy_original`, etc.)"""
    save_topk_best: int = 1
    """number of best-metric checkpoints to protect from deletion"""
    ...
    def post_init(self):
        if 0 < self.save_limit and self.save_topk_best >= self.save_limit:
            raise ValueError(
                f"`save_topk_best` ({self.save_topk_best}) must be smaller than `save_limit` ({self.save_limit}) "
                "because the latest checkpoint also counts toward the limit."
            )
...
class PPOConfig:
    ...
    def post_init(self):
        ...
        self.worker.concat_responses = self.algorithm.concat_responses  # 传递 concat_responses 到 worker
