...
def create_dataloader(config: DataConfig, tokenizer: PreTrainedTokenizer, processor: Optional[ProcessorMixin]) -> None:
    ...
    for val_file in config.val_files:
        val_set_name = val_file.split("/")[-1].split("@")[0]
        ...
        val_datasets.append({"name": val_set_name, "dataloader": val_dataloader})

    assert len(train_dataloader) >= 1
    for val_dataset in val_datasets:
        assert len(val_dataset["dataloader"]) >= 1
    print(f"Size of train dataloader: {len(train_dataloader)}")
    for val_dataset in val_datasets:
        print(f"Size of {val_dataset['name']} dataloader: {len(val_dataset['dataloader'])}")
    return train_dataloader, val_datasets
