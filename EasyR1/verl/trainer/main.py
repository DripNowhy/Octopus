...
from ..workers.reward import BatchFunctionRewardManager, SequentialFunctionRewardManager, SelectiveBatchFunctionRewardManager
...
class Runner:
    ...
    def run(self, config: PPOConfig):
        ...
        elif config.worker.reward.reward_type == "selective_batch":
            RewardManager = SelectiveBatchFunctionRewardManager
        else:
        ...
        # 验证时的 reward manager（可能使用不同的函数和类型）
        # 如果设置了 val_reward_function，创建单独的 validation reward config
        if config.worker.reward.val_reward_function is not None:
            from copy import deepcopy
            val_reward_config = deepcopy(config.worker.reward)
            # 交换 reward_function 和 val_reward_function
            ...
            val_reward_config.reward_function_kwargs = config.worker.reward.val_reward_function_kwargs
            # validation 时通常不需要样本选择，使用 BatchFunctionRewardManager
            ValRewardManager = BatchFunctionRewardManager
            RemoteValRewardManager = ray.remote(ValRewardManager).options(num_cpus=config.worker.reward.num_cpus)
            val_reward_fn = RemoteValRewardManager.remote(val_reward_config, tokenizer)
        else:
            # 如果没有指定 val_reward_function，使用相同的配置
            val_reward_fn = RemoteRewardManager.remote(config.worker.reward, tokenizer)

        train_dataloader, val_dataloader_list = create_dataloader(config.data, tokenizer, processor)
