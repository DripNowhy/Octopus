...
def remove_obsolete_ckpt(
    ...
    protected_steps: Optional[list[int]] = None,
):
    """
    ...
    protected_steps_set = set(protected_steps or [])
    if best_global_step is not None:
        protected_steps_set.add(best_global_step)
    ...
    protected_existing = []
    unprotected_steps = []
    for step in ckpt_global_steps:
        if step in protected_steps_set:
            protected_existing.append(step)
        else:
            unprotected_steps.append(step)

    remaining_slots = max(num_ckpt_to_keep - len(protected_existing), 0)
    keep_steps = set(protected_existing + unprotected_steps[:remaining_slots])
