...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        if os.path.isdir(data_path):
            ...
            if not files:
                raise ValueError(f"数据集目录为空: {data_path}")
            
            # 检查是否是 Arrow 格式的数据集（包含 .arrow 文件和 dataset_info.json）
            has_arrow_files = any(f.endswith('.arrow') for f in files)
            has_dataset_info = 'dataset_info.json' in files or 'state.json' in files
            
            if has_arrow_files and has_dataset_info:
                # 使用 load_from_disk 加载 Arrow 格式的数据集
                try:
                    loaded_dataset = load_from_disk(data_path)
                    # 如果加载的是 DatasetDict，尝试选择对应的 split
                    if hasattr(loaded_dataset, 'keys') and data_split in loaded_dataset.keys():
                        self.dataset = loaded_dataset[data_split]
                    elif isinstance(loaded_dataset, dict) and data_split in loaded_dataset:
                        self.dataset = loaded_dataset[data_split]
                    else:
                        # 如果是单个 Dataset，直接使用
                    ...
                    raise RuntimeError(
                        f"加载 Arrow 格式数据集失败 (路径: {data_path}, 分割: {data_split}): {str(e)}"
                    ) from e
            else:
                # 使用传统方式加载（JSON/JSONL 等）
                file_type = os.path.splitext(files[0])[-1][1:].replace("jsonl", "json")
                try:
                    self.dataset = load_dataset(file_type, data_dir=data_path, split=data_split)
                except Exception as e:
                    raise RuntimeError(
                        f"加载数据集失败 (路径: {data_path}, 类型: {file_type}, 分割: {data_split}): {str(e)}"
                    ) from e
            ...
            try:
                ...
                raise RuntimeError(
                    f"加载数据集失败 (文件: {data_path}, 类型: {file_type}, 分割: {data_split}): {str(e)}"
                ) from e
        else:
            ...
            try:
                ...
                raise RuntimeError(
                    f"加载远程数据集失败 (数据集: {data_path}, 分割: {data_split}): {str(e)}"
                ) from e
