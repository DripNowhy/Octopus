...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto) -> dict[str, Any]:
        ...
        if "kl_mask" in data.batch.keys():
            select_keys.append("kl_mask")
        non_tensor_select_keys = ["multi_modal_inputs"]
        ...
        for _ in range(self.config.ppo_epochs):
            ...
            for mini_batch in mini_batches:
                ...
                for micro_batch in micro_batches:
                    ...
                    y1_kl_coef = getattr(self.config, "y1_kl_coef", 0.0)
                    if y1_kl_coef > 0 and "kl_mask" in model_inputs and "ref_log_probs" in model_inputs:
                        kl_mask = model_inputs["kl_mask"]
                        if kl_mask.sum() > 0:
                            ref_log_probs_y1 = model_inputs["ref_log_probs"]
                            y1_kld = compute_kl(
                                log_probs=log_probs,
                                ref_log_probs=ref_log_probs_y1,
                                kl_penalty=self.config.kl_penalty,
                            )
                            y1_kl_loss = average_loss(y1_kld, kl_mask, mode=self.config.loss_avg_mode)
                            loss = loss + y1_kl_loss * y1_kl_coef
                            metrics["actor/y1_kl_loss"] = y1_kl_loss.detach().item()
                            metrics["actor/y1_kl_coef"] = y1_kl_coef
