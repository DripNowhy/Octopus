...
class FSDPWorker(Worker):
    ...
    def _init_dist_mesh(self, config: Union[ActorConfig, CriticConfig], role: Literal["actor", "critic"]):
        ...
        if self.config.rollout.n > 1:
            # 如果启用了 concat_responses，实际样本数是 2n 倍（因为 select_top_k=2）
            # 否则是 n 倍
            if self.config.concat_responses:
                # 选择后的样本数是 2n，所以 global_batch_size 也要乘 2n
                multiplier = self.config.rollout.n * 2
                config.global_batch_size *= multiplier
                self.print_rank0(f"{role} will use global batch size {config.global_batch_size} (multiplier={multiplier} for concat_responses).")
            else:
                # 正常情况，乘 n
    ...
    def generate_sequences_train(self, prompts: DataProto):
        """训练时的序列生成，使用带 stop string 的采样参数"""
        assert self._has_rollout
        ...
        prompts = self.rollout_sharding_manager.preprocess_data(prompts)
        output = self.rollout.generate_sequences_train(prompts=prompts)
        output = self.rollout_sharding_manager.postprocess_data(output)
